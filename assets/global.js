/**
 * Tulips Custom Theme - JavaScript
 * Version: 1.0.0
 * Theme: Tulips Custom Theme for Tulips Hygiene
 * Author: Tulips Hygiene Development Team
 * Build Date: 2024-01-07
 */

/* Tulips Theme Scripts */
...
const sectionsOfHeaderGroup = document.querySelectorAll('.shopify-section-group-header-group');
...
if (isHeaderTransparent) {
  ...
  if (sectionPageBanner) {
    const sectionIndex = Array.prototype.indexOf.call(main.children, sectionPageBanner);
    const hasMedia = sectionPageBanner.querySelectorAll('.media').length > 0;

    if (sectionIndex === 0 && !hasMedia) {
      header.classList.remove('is-transparent');
    }

    /* CHECK and if not needed, remove it *
    const sectionPageBannerContent = sectionPageBanner?.querySelector(".content");
    if (sectionPageBannerContent) {
      sectionPageBanner.style.paddingBlockStart = `calc(${parseInt(header.offsetHeight)}px + 1.2rem)`;
    }
    */
  }
}
...
if (pageCollection) {
  ...
  const sectionCollection = pageCollection.querySelector('.section-collection');
  if (sectionCollection) {
    const sectionIndex = Array.prototype.indexOf.call(main.children, sectionCollection);

    const handleScroll = () => {
      if (sectionIndex === 0 && window.scrollY === 0) {
        header.classList.remove('is-transparent');
      }
    }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  lazyImages.forEach(img => {
    // add opacity 0 to all images with loading lazy
  ...
  function handleLazyLoadedImage(img) {
    ...
    // first add opacity manually and start transition
    img.style.cssText = `opacity: 1; transition: opacity ${transitionDuration}ms ease-in-out !important;`;
    setTimeout(() => {
...
function setRootCustomProperties() {
  ...
  // sections between header and main - top/bottom
  if (isHeaderTransparent) {
    const childrenOfBody = Array.from(document.body.children);
    const indexHeader = childrenOfBody.indexOf(document.querySelector('header'));
    const indexMain = childrenOfBody.indexOf(document.querySelector('main'));

    const sectionsBetweenHeaderAndMain = childrenOfBody.slice(indexHeader + 1, indexMain);

    if (sectionsBetweenHeaderAndMain.length === 0) {
      document.documentElement.style.setProperty('--header-to-main-sections-height', `0px`);
    } else {
      // first section between header and main
      const firstSection = sectionsBetweenHeaderAndMain.find(section => section.tagName === 'SECTION');
      // check header is still transparent
      const isHeaderStillTransparent = header.classList.contains('is-transparent');
      if (firstSection && isHeaderStillTransparent) {
        firstSection.style.marginBlockStart = `${header.offsetHeight}px`;
      }

      // minimum top value of the sections
      const sectionsBetweenTop = sectionsBetweenHeaderAndMain.reduce((acc, section) => {
        return Math.min(acc, section.getBoundingClientRect().top);
      }, Infinity);
      // maximum bottom value of the sections
      const sectionsBetweenBottom = sectionsBetweenHeaderAndMain.reduce((acc, section) => {
        return Math.max(acc, section.getBoundingClientRect().bottom);
      }, -Infinity);
      // calc height of the sections
      const sectionsBetweenHeight = sectionsBetweenBottom - sectionsBetweenTop;

      document.documentElement.style.setProperty('--header-to-main-sections-height', `${sectionsBetweenHeight}px`);
    }
  }
...
const isAnnouncementBarAfterHeader =
  announcementBar?.parentElement?.previousElementSibling?.classList.contains('section--header');
...
class TulipsMenuDrawer extends HTMLElement {
  constructor() {
    super();

    this.details = this.querySelector("details");
    this.summary = this.querySelector("summary");
    this.drawer = this.querySelector(".js-drawer, .tulips-js-drawer");
    this.btnsCloseDrawer = this.querySelectorAll(
      ".js-btn-close-drawer, .tulips-js-btn-close-drawer"
    );
    this.btnsCloseDrawer = [...this.btnsCloseDrawer].filter(
      btnCloseDrawer =>
        this.drawer === btnCloseDrawer.closest(".js-drawer, .tulips-js-drawer")
    );
    this.elementToFocus =
      this.querySelector(".js-drawer-focus-element, .tulips-js-drawer-focus-element") ||
      this.btnsCloseDrawer[0];
    this.toggleButtons = [this.summary, ...this.btnsCloseDrawer];
    this.isParentDrawerOpen = false;
    this.predictiveSearch = this.querySelector("predictive-search");

    this.HeaderDrawer = document.querySelector("tulips-header-drawer, header-drawer");
    ...
    this.addEventListener("keydown", e => {
      ...
      const isDrawerOpen =
        this.details.classList.contains("tulips-menu-opening");
  ...
  toggleDrawer() {
    ...
    if (!isDetailsOpen) {
      ...
      this.toggleButtons.forEach(toggleButton => {
        toggleButton.classList.add("menu-is-open", "tulips-menu-is-open");
        toggleButton.setAttribute("aria-expanded", true);
      });
      this.details.classList.add("menu-opening", "tulips-menu-opening");
      headerDiv.classList.add("menu-open", "tulips-menu-open");
      const bodyHasOverflow = body.style.overflow === "hidden";
      if (bodyHasOverflow) {
        this.parentDrawer = this.closest(".js-drawer, .tulips-js-drawer");
        ...
        body.classList.add("drawer--is-open", "tulips-drawer--is-open");
      }
      var searchInput = this.querySelector("#search-desktop");
      if (searchInput) {
        searchInput.focus();
        if (headerDiv) {
          headerDiv.classList.remove("menu-open", "tulips-menu-open");
        }
      }
    } else {
      this.details.classList.remove("menu-opening", "tulips-menu-opening");
      this.toggleButtons.forEach(toggleButton => {
        toggleButton.classList.remove("menu-is-open", "tulips-menu-is-open");
        toggleButton.setAttribute("aria-expanded", false);
      });
      body.style.overflow = "";
      body.classList.remove("drawer--is-open", "tulips-drawer--is-open");
    ...
    const handleDropdownTransition = e => {
      ...
      this.details.classList.remove("drawer-transitioning", "tulips-drawer-transitioning");
    ...
    this.drawer.addEventListener(
      "transitionstart",
      e => {
        this.details.classList.add("drawer-transitioning", "tulips-drawer-transitioning");
      },
      {
        once: true
      }
  ...
  closeAllOtherDrawers() {
    const allDrawers = document.querySelectorAll(".js-drawer, .tulips-js-drawer");
  ...
  toggleTrapFocus(container = this.drawer) {
    ...
    document.addEventListener("focusin", e => {
      ...
      const openDrawer = e.target.closest(
        "details.menu-opening .js-drawer, details.tulips-menu-opening .tulips-js-drawer"
      );
      const nestedOpenDrawer = this.details.querySelector(
        "details.menu-opening, details.tulips-menu-opening"
      );
...
customElements.define("tulips-menu-drawer", TulipsMenuDrawer);

class TulipsHeaderDrawer extends TulipsMenuDrawer {
  constructor() {
    super();
...
customElements.define("tulips-header-drawer", TulipsHeaderDrawer);

class TulipsDesktopDrawer extends TulipsMenuDrawer {
  constructor() {
    super();
...
customElements.define("tulips-desktop-drawer", TulipsDesktopDrawer);

class TulipsSearchDrawer extends TulipsMenuDrawer {
  constructor() {
    super();
    ...
    this.btn.addEventListener("click", () => {
      const headerDrawerDetails = document.querySelector("tulips-header-drawer details, header-drawer details");
      if (headerDrawerDetails) {
        headerDrawerDetails.removeAttribute("open");
        headerDrawerDetails.classList.remove("menu-opening", "tulips-menu-opening");
      }
    });
  }
}
customElements.define("tulips-search-drawer", TulipsSearchDrawer);
...
customElements.define("tulips-quantity-input", QuantityInput);

class TulipsModalOpener extends HTMLElement {
  constructor() {
    super();
...
customElements.define("tulips-modal-opener", TulipsModalOpener);

class TulipsModalDialog extends HTMLElement {
  constructor() {
    super();
...
customElements.define("tulips-modal-dialog", TulipsModalDialog);
...
class TulipsDeferredMedia extends HTMLElement {
  constructor() {
    super();
...
customElements.define("tulips-deferred-media", TulipsDeferredMedia);

/**
 * Additions
 */

class TulipsLocalizationForm extends HTMLElement {
  constructor() {
    super();
...
customElements.define("tulips-localization-form", TulipsLocalizationForm);

class TulipsAccordionDefault extends HTMLElement {
  constructor() {
    super();
  ...
  collapseInactiveItems() {
    ...
    const allSummaryElements = document.querySelectorAll(
      "accordion-default summary, tulips-accordion-default summary"
    );
    allSummaryElements.forEach(summary => {
      const accordionDefault = summary.closest("accordion-default, tulips-accordion-default");
...
// Define tulips-accordion-default first
customElements.define("tulips-accordion-default", TulipsAccordionDefault);

// Create an alias class for accordion-default that extends tulips-accordion-default
class AccordionDefault extends TulipsAccordionDefault {
  constructor() {
    super();
  }
}
...
const nav = document.querySelector(".js-nav, .tulips-js-nav");

nav?.addEventListener("click", function (e) {
  const isHoverDisabled = matchMedia("(hover: none)").matches;
  const isLink = e.target.classList.contains("js-nav-link") || e.target.classList.contains("tulips-js-nav-link");
  ...
  const activeLinkItem = linksList.querySelector(
    ".js-nav-item.is-active, .tulips-js-nav-item.is-active"
  );
...
let navItems = document.querySelectorAll(
  ".js-nav-item.has-dropdown:not(.dropdown), .tulips-js-nav-item.has-dropdown:not(.dropdown)"
);
...
["DOMContentLoaded", "resize"].forEach(eventType => {
  window.addEventListener(eventType, () => {
    ...
    navItems.forEach(navItem => {
      const dropdown = navItem.querySelector(".js-dropdown, .tulips-js-dropdown");
...
if (megaMenu) {
  ...
  function toggleHeaderTransparency() {
    ...
    if (hasVisibleDropdown && isHeaderTransparent) {
...
if (isHeaderTransparent && megaMenu.length > 0) {
  ...
  let isStillTransparent = header.classList.contains("is-transparent");
  ...
  megaMenu.forEach(link => {
    ...
    const handleMouseLeaveDropdown = () => {
      if (!isDropdownOpen && isStillTransparent) {
...
class TulipsDropdownInput extends HTMLElement {
  constructor() {
    super();
...
customElements.define("tulips-dropdown-input", TulipsDropdownInput);
  ...
  class ProductCard extends HTMLElement {
    ...
    initForm() {
      ...
      this.addToCartForm = this.querySelector(
        ".mobile_hidden_atc form.product-card__add-to-cart--form"
      );
      ...
      /** Event Listeners */
      if (this.addToCartForm && !this.addToCartForm._hasSubmitListener) {
        this.addToCartForm.addEventListener(
          "submit",
          this.submitAddToCartForm.bind(this)
        );
        this.addToCartForm._hasSubmitListener = true;
      }
    }
...
// //MOILE MENU LINKS SETTING
// document.querySelectorAll(".menu-button-with-link").forEach(button => {
//   // Navigate to the page when the button is clicked
//   button.addEventListener("click", function (event) {
//     event.stopPropagation(); // Prevent interference with hover
...
//   // Show submenu on hover (mouseenter)
//   button.addEventListener("mouseenter", function () {
//     let submenu = this.closest("li")?.querySelector("ul");
//     if (submenu) {
//       submenu.style.display = "block"; // Make sure submenu appears
//     }
//   });

//   // Hide submenu when mouse leaves (mouseleave)
