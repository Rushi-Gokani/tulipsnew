...
const cartDrawer = document.querySelector('cart-drawer, tulips-cart-drawer');
  ...
  updateQuantity(line, quantity, name) {
    ...
    fetch(`${routes.cart_change_url}`, {
      ...
      .then(state => {
        ...
        // Re-initialize Swiper for cart drawer recommendations after cart update
        if (typeof window.initCartDrawerRecommendationsSwiper === 'function') {
          window.initCartDrawerRecommendationsSwiper();
        }
      })
...
class CartDrawer extends HTMLElement {
  ...
  open(opener) {
    ...
    // Re-initialize Swiper for cart drawer recommendations when drawer opens
    if (typeof window.initCartDrawerRecommendationsSwiper === 'function') {
      setTimeout(() => {
        window.initCartDrawerRecommendationsSwiper();
      }, 300);
  ...
  renderContents(response, open = true) {
    ...
    // Re-initialize Swiper for cart drawer recommendations after render
    if (typeof window.initCartDrawerRecommendationsSwiper === 'function') {
      window.initCartDrawerRecommendationsSwiper();
    }
    ...
    // DISABLED: Cart drawer opening is disabled to show notification instead
    console.log("Cart drawer renderContents called but opening disabled");
    // this.open();
  }
